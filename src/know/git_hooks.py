...
class GitHookManager:
    ...
    def install_post_commit(self) -> bool:
        ...
        if hook_path.exists():
            ...
            if "know" in content:
                print("⚠ know hook already installed")
                return True
            
            # Backup existing hook
            backup = hook_path.with_suffix(".backup")
            hook_path.rename(backup)
            print(f"ℹ Backed up existing hook to {backup}")
    ...
    def uninstall_post_commit(self) -> bool:
        ...
        if "know" not in content:
            print("⚠ Hook doesn't appear to be from know, not removing")
            return False
    ...
    def status(self) -> None:
        ...
        for hook_name in ["post-commit", "pre-commit"]:
            ...
            if hook_path.exists():
                content = hook_path.read_text()
                if "know" in content:
                    print(f"  ✓ {hook_name}: installed by know")
                else:
                    print(f"  ⚠ {hook_name}: exists (not from know)")
            else:
