...
class DocGenerator:
    ...
    def generate_all(self) -> None:
        ...
        self.generate_readme(structure)
    ...
    def generate_readme(self, structure: Dict[str, Any], output: Optional[str] = None) -> Path:
        ...
        lines.extend([
            ...
            "---",
            "",
            f"*This README was generated by [know](https://github.com/vic/know-cli) v{self.config.project.version}*",
            "",
        ...
        if output:
            ...
            path = self.config.root / "README.md"
        
        # Preserve existing README content if it exists
        if path.exists():
            existing = path.read_text()
            # Find marker for auto-generated section
            if "<!-- KNOW-START -->" in existing:
                before = existing.split("<!-- KNOW-START -->")[0]
                after = ""
                if "<!-- KNOW-END -->" in existing:
                    after = existing.split("<!-- KNOW-END -->")[1]
                content = f"{before}<!-- KNOW-START -->\n{content}\n<!-- KNOW-END -->{after}"
            else:
                content = f"{existing}\n\n<!-- KNOW-START -->\n{content}\n<!-- KNOW-END -->"
