...
def test_hooks_install_post_commit_script_is_non_mutating(tmp_git_project):
    from know.cli import cli
    ...
    hook = (tmp_git_project / ".git" / "hooks" / "post-commit").read_text(encoding="utf-8")
    assert "know warm --json" in hook
    assert "know update" not in hook
    assert "git add" not in hook
    assert "git commit" not in hook
    assert "--amend" not in hook


def test_hooks_install_updates_stale_managed_post_commit_hook(tmp_git_project):
    from know.cli import cli

    hooks_dir = tmp_git_project / ".git" / "hooks"
    (hooks_dir / "post-commit").write_text(
        "#!/bin/bash\n"
        "# Auto-generated by know - stale hook\n"
        "know update --quiet\n"
        "git add docs/\n"
    ...
    hook = (hooks_dir / "post-commit").read_text(encoding="utf-8")
    assert "know warm --json" in hook
    assert "know update" not in hook
    assert "git add" not in hook
    assert "git commit" not in hook
